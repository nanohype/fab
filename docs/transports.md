...
| **System prompt**             | Uploaded at deploy time; baked into the agent.                                        | Rebuilt per session from `buildSystemPrompt`.                                                                                                         | Built per session; written to a per-session file passed as `--append-system-prompt-file`.                                                              |
| **Session resume**            | First-class via `client.beta.sessions.*`.                                             | Not supported. `resumeSession` returns a stand-in that throws on first input; continuation means a new role session seeded with the prior transcript. | `claude -p --resume <session-id>` opens a fresh subprocess against the same session.                                                                   |
...
| `FAB_CLAUDE_MCP_DIR`     | `os.tmpdir()`          | Directory for per-session files: the MCP config JSON and the system prompt                                                                                                       |
...
- **Quality-check rubric.** The overlay chain (`$FAB_SKILLS_DIR` → `~/.fab/skills/` → `<cwd>/.fab/skills/` → bundled `fab/skills/`) resolves the rubric the gate roles and the external reviewer grade from by the same rules on every transport, on the machine that builds the prompt: fab's own process for `sdk` and `claude-cli`, the deploying machine at `fab deploy` for `managed-agents`, and the session pod for `sdk-k8s`, where only the layers inside the session image apply.
...
- **Role skills attach only on `managed-agents`.** They resolve when `fab deploy` or `fab skills upload` runs; the `sdk`, `claude-cli` and `sdk-k8s` transports load no role skill, so an overlay file for one (`pr-review.append.md`, for example) reaches no agent there. The quality-check rubric is not a role skill: every transport builds it into the grading roles' prompts.
