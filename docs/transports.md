...
| **Cost tracking**             | `span.model_request_end` per request, summed while the session runs.                 | `span.model_request_end` per request, translated from each assistant message's `usage` block, plus `total_cost_usd` on the result. | Same as the SDK runtime — the two speak one message shape and share one translator. |
