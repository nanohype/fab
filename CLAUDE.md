...
A TypeScript CLI that deploys and orchestrates a team of 78 Claude managed agents via the Anthropic Managed Agents API. The roster is organized around factory phases — workflow code in `src/workflows.ts` fans out across phase-scoped multiagent sessions. There is no top-level coordinator agent; Managed Agents caps a multiagent roster at 20 unique agents and does not nest coordinators, so each phase runs as its own session and workflow code orchestrates across them.
...
- `src/prompts.ts` — `buildSystemPrompt()` augmentation layer: appends journal, self-eval, repo, sprint, revision, and factory-production-standards preamble sections based on state + group. Every merge-gate role and `external-reviewer` also get the Quality rubric depth section, and `external-reviewer` gets `QUALITY_RUBRIC`. The Build Verification Protocol dispatches language-specific commands via `LANGUAGE_TOOLCHAIN[state.projectLanguage]`.
- `src/rubric.ts` — `qualityRubricDepth()`: the quality-check rubric's dimension sections (the resolved base from `## 0. Apply the published standards` up to `## Output format`, then every `quality-check.append.md`), resolved through the overlay chain on each call. The rubric's run instructions and report format stay out: a gate role emits the block its own prompt declares.
- `src/standards.ts` — factory production policies. Two layers: (1) **public bar** loaded at module init from the vendored `standards/*.json` — `LANGUAGE_TOOLCHAIN` from `language-toolchain.json`, `LLM_MODELS` + `MODEL_TIERS` from `llm-policy.json`, which `LLM_POLICY` renders and `src/advisor.ts` + every role's `model` resolve against, and `COVERAGE_FLOOR` from `testing-rubric.json`, which `PRODUCTION_BAR` and the Build Verification Protocol render; (2) **private choreography** declared here as markdown blobs: `FOUR_PHASE_CONTRACT`, `VERSION_CURRENCY_POLICY`, `EVIDENCE_CONTRACT`, `QUALITY_RUBRIC` (per-role assignments + N/A criteria for the public dimension names; what each grade means is the quality-check rubric), `IAC_BY_TARGET`, `PLATFORM_TENANT_CONTRACT`, `PRODUCTION_BAR` (9 pass-fail requirements with the specific REJECT criteria; a failure is a REJECT verdict, not a grade cap), `COMMIT_PR_POLICY`, `MERGE_GATE_CONTRACT`, `FACTORY_PREAMBLE` (assembled), `CODE_GATE_ROLES`, `DOCS_GATE_ROLES`. The public bar JSON is the [Platform Reference](../nanohype/docs/platform-reference.md); external clients see the guardrails, only fab knows the depth.
...
- **Latest versions first** (`VERSION_CURRENCY_POLICY`) — every manifest entry is checked against its language registry (npm, proxy.golang.org, PyPI, crates.io, Maven Central, NuGet) at build-verifier time. Entries more than one major behind current stable without an adjacent `@pin <reason>` annotation = REJECT. EOL language runtimes (Python ≤3.9, Node ≤18 post-2025-04, Go ≤1.20) = REJECT regardless of `@pin`. Intake-analyst enforces this at brief-submission time too.
...
- **Quality rubric** (`QUALITY_RUBRIC`) — 10 dimensions, each assigned to one gate role. What each grade means is the quality-check rubric (`skills/quality-check.md` plus every `quality-check.append.md` in the overlay chain): gate roles and `external-reviewer` receive its dimension sections through `src/rubric.ts`, and an interactive quality check reads the whole file. Both grade from the same definitions with different graders, so their grades can differ. Every verdict ends with a `QUALITY_GRADES:` block scoped to the role's dimensions. `QUALITY_DIMENSIONS` + `QUALITY_DIMENSION_OWNERS` in `standards.ts` are the data behind the prose: pr-reviewer grades architecture/patterns/frontend/code_quality; qa-security grades security/systems/ai_systems; build-verifier grades testing; artifact-auditor grades documentation/consistency. The owners map partitions the dimension list, and `__tests__/quality-contract.test.ts` holds the role prompts, the parser, and those constants to each other — a role that declares a block `parseQualityGrades` can't read fails there rather than silently grading nothing. The same test holds the baseline's `QUALITY_GRADES:` block to `QUALITY_DIMENSIONS` and `QUALITY_GRADE_TOKENS`, and its N/A table to `QUALITY_RUBRIC`'s. The `external-reviewer` role (lab group) grades all 10 cold (no internal verdicts, no prior context) as a calibration check — `compareGrades` blocks release on >1-letter drift between internal and external per dimension, and a cold review that leaves a dimension without a readable line fails calibration.
- **IaC by deploy_target** — k8s-native is the default: Helm chart + ApplicationSet entry into `nanohype/eks-gitops` + `Platform` CR on `eks-agent-platform`. Cloud substrate gaps land as new components in `nanohype/landing-zone` (OpenTofu/Terragrunt). Cluster addons land in the gitops repo. Escape hatches (require architecture-artifact justification): `aws-lambda`→AWS CDK, `fly`→fly.toml, `vercel`→vercel.json, `cloudflare`→Wrangler. AWS CDK is reachable only via the `aws-lambda` escape hatch — the default AWS path is k8s-native (EKS via landing-zone + eks-gitops + eks-agent-platform).
- **Platform tenant contract** (`PLATFORM_TENANT_CONTRACT`) — every k8s-native deliverable ships as a Platform tenant: `<app>/chart/` Helm chart with per-env values, `<app>/gitops/applicationset-entry.yaml`, `<app>/platform.yaml` (CR), optional `agentfleet.yaml` for AI workloads. The Platform CR declares the tenant's stateful substrate in `spec.datastores` (relational/keyValue/objectStore/queue/cache/stream) — the generic tenant-substrate module provisions each store and the operator generates the scoped IAM to reach it, so agents never hand-write a per-app substrate component. Non-datastore AWS capabilities (SES send, EventBridge Scheduler) are declared in `spec.identity.capabilities`, not hand-written policies via `extraPolicyArns`. Required OTel resource attrs: `agents.tenant`, `agents.platform`, plus `agents.model_family` + `agents.model_id` for AI workloads. The operator owns identity: it provisions the tenant IAM role, creates the `tenant-runtime` ServiceAccount, and binds it via a Pod Identity association; agents do NOT scaffold IAM roles inline, create a chart-owned ServiceAccount, or annotate any SA with a role ARN — the chart references `tenant-runtime` (`serviceAccount.create: false`).
- **LLM policy** — Claude-primary via AWS Bedrock. SDK per language (TS `@aws-sdk/client-bedrock-runtime`, Python `boto3`, Go `aws-sdk-go-v2/service/bedrockruntime`, Rust `aws-sdk-bedrockruntime`, Java `software.amazon.awssdk:bedrockruntime`, C# `AWSSDK.BedrockRuntime`). IAM-role auth. Default `claude-sonnet-5`, opus for complex reasoning, haiku for classification. Prompt caching mandatory.
- **Production bar** (9 requirements, non-negotiable — distinct from the rubric's 10 dimensions) — tests, observability, security, reliability, cost, docs, CI, code-shape, versions. See `PRODUCTION_BAR` in standards.ts for full text.
