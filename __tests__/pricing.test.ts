...
describe('estimateCost', () => {
  it('prices input + output at the resolved model tier', () => {
    expect(
      estimateCost({ input_tokens: 1_000_000, output_tokens: 1_000_000 }, 'claude-sonnet-4-6'),
    ).toBeCloseTo(18); // 3 + 15
    expect(
      estimateCost({ input_tokens: 1_000_000, output_tokens: 1_000_000 }, 'claude-opus-4-8'),
    ).toBeCloseTo(30); // 5 + 25
    expect(
      estimateCost({ input_tokens: 1_000_000, output_tokens: 1_000_000 }, 'claude-haiku-4-5'),
    ).toBeCloseTo(6); // 1 + 5
  });
  ...
  it('treats null / absent cache fields as zero', () => {
    expect(
      estimateCost(
        { input_tokens: 1_000_000, output_tokens: 0, cache_read_input_tokens: null },
        'claude-sonnet-4-6',
      ),
    ).toBeCloseTo(3);
  });
